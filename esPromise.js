...
async function travelThroughHistory(n = 95) {
  ...
  try {
    while (eventiUnici.size < 95) {
      const eventRequest = Array.from({ length: n }, () => getPastEvent());
      const result = await Promise.all(eventRequest);
      result.forEach(event => {
        if (!eventiUnici.has(event.name)) {
    ...
    // request.forEach(getPastEvent()((item,i,arr) => {arr[i]= request.push(item)}))
    const arr = Array.from(eventiUnici.values()).filter(event => event.year < 2000).sort((a, b) => a.year - b.year);
    //eventiUnici.sort((a, b) => a.year - b.year).filter((events, i, arr) => i === arr.findIndex(event => event.name === events.name)).filter(events => events.year < 2000);
    console.log(arr);
